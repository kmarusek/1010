...
<div class="fl-custom-query fl-loop-data-source" data-source="custom_query">
	<div id="fl-builder-settings-section-filter" class="fl-builder-settings-section">
		...
		<?php foreach ( FLBuilderLoop::post_types() as $slug => $type ) : ?>
			...
			// Filter Rule
			FLBuilder::render_settings_field('filter_rule', array(
				'type'          => 'select',
			...
			foreach ( $taxonomies as $tax_slug => $tax ) {
				...
				FLBuilder::render_settings_field( 'tax_' . $slug . '_' . $tax_slug, array(
					...
					'data'          => $tax_slug,
					'label'         => $tax->label,
					'help'          => sprintf( __( 'Enter a list of %1$s.', 'uabb' ), $tax->label ),
					/*'matching'      => true,*/
				), $settings );
			}

			FLBuilder::render_settings_field('display_cat_desc', array(
				'type'          => 'select',
			...
			FLBuilder::render_settings_field('display_empty_cat', array(
				'type'          => 'select',
