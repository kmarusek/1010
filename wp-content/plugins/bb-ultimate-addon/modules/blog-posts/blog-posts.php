			...
			if ( ! empty( $object_taxonomies ) ) {
				...
				if ( count( $category_detail ) > 0 ) {
					...
					for ( $j = 0; $j < count( $category_detail ); $j++ ) {
						?>
				<span class="uabb-cat-links"><a href="<?php echo get_term_link( $category_detail[ $j ]->term_id ); ?>" rel="category tag"><?php echo $category_detail[ $j ]->name; ?></a></span>
						<?php
						echo ( count( $category_detail ) != $j + 1 ) ? trim( ',&nbsp;' ) : '';
					}
				}
			}
		}
