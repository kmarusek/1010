...
class BlogPostsModule extends FLBuilderModule {
    ...
    function render_new_data( $data ) {
		...
		foreach ( $data as &$node ) {
			...
			if ( isset( $node->settings->type ) && 'blog-posts' === $node->settings->type ) {

				if( isset( $node->settings->overall_padding ) &&  !isset( $node->settings->overall_padding_dimension_top ) &&  !isset( $node->settings->overall_padding_dimension_bottom ) &&  !isset( $node->settings->overall_padding_dimension_left ) &&  !isset( $node->settings->overall_padding_dimension_right ) ) {
                     
                    $value = "";
                    $value = str_replace("px","", $node->settings->overall_padding );
                    
                    $output = array();
                    $uabb_default = array_filter( preg_split("/\s*;\s*/", $value) );
                    
                    $node->settings->overall_padding_dimension_top    = '0';
                    $node->settings->overall_padding_dimension_bottom = '0';
                    $node->settings->overall_padding_dimension_left   = '0';
                    $node->settings->overall_padding_dimension_right  = '0';
                    ...
                    for ($i=0; $i < count( $output ); $i++) { 
                        switch ( $output[$i][0] ) {
                            case 'padding-top':
                               $node->settings->overall_padding_dimension_top    = (int)$output[$i][1];
                                break;
                            case 'padding-bottom':
                                $node->settings->overall_padding_dimension_bottom = (int)$output[$i][1];
                                break;
                            case 'padding-right':
                                $node->settings->overall_padding_dimension_right  = (int)$output[$i][1];
                                break;
                            case 'padding-left':
                                $node->settings->overall_padding_dimension_left   = (int)$output[$i][1];
                                break;
                            case 'padding':
                                $node->settings->overall_padding_dimension_top    = (int)$output[$i][1];
                                $node->settings->overall_padding_dimension_bottom = (int)$output[$i][1];
                ...
                if( isset( $node->settings->masonary_padding ) &&  !isset( $node->settings->masonary_padding_dimension_top ) &&  !isset( $node->settings->masonary_padding_dimension_bottom ) &&  !isset( $node->settings->masonary_padding_dimension_left ) &&  !isset( $node->settings->masonary_padding_dimension_right ) ) {
                     
                    $value = "";
                    $value = str_replace("px","", $node->settings->masonary_padding );
                    
                    $output = array();
                    $uabb_default = array_filter( preg_split("/\s*;\s*/", $value) );
                    
                    $node->settings->masonary_padding_dimension_top    = '0';
                    $node->settings->masonary_padding_dimension_bottom = '0';
                    $node->settings->masonary_padding_dimension_left   = '0';
                    $node->settings->masonary_padding_dimension_right  = '0';
                    ...
                    for ($i=0; $i < count( $output ); $i++) { 
                        switch ( $output[$i][0] ) {
                            case 'padding-top':
                               $node->settings->masonary_padding_dimension_top    = (int)$output[$i][1];
                                break;
                            case 'padding-bottom':
                                $node->settings->masonary_padding_dimension_bottom = (int)$output[$i][1];
                                break;
                            case 'padding-right':
                                $node->settings->masonary_padding_dimension_right  = (int)$output[$i][1];
                                break;
                            case 'padding-left':
                                $node->settings->masonary_padding_dimension_left   = (int)$output[$i][1];
                                break;
                            case 'padding':
                                $node->settings->masonary_padding_dimension_top    = (int)$output[$i][1];
                                $node->settings->masonary_padding_dimension_bottom = (int)$output[$i][1];
				...
				if ( isset( $node->settings->meta_font_size['small']) && !isset( $node->settings->meta_font_size_unit_responsive ) ) {
					$node->settings->meta_font_size_unit_responsive = $node->settings->meta_font_size['small'];
				}
				if( isset( $node->settings->meta_font_size['medium']) && !isset( $node->settings->meta_font_size_unit_medium ) ) {
					$node->settings->meta_font_size_unit_medium = $node->settings->meta_font_size['medium'];
				}
				if( isset( $node->settings->meta_font_size['desktop']) && !isset( $node->settings->meta_font_size_unit ) ) {
					$node->settings->meta_font_size_unit = $node->settings->meta_font_size['desktop'];
				}

				if ( isset( $node->settings->meta_line_height['small']) && isset( $node->settings->meta_font_size['small'] ) && $node->settings->meta_font_size['small'] != 0 && !isset( $node->settings->meta_line_height_unit_responsive ) ) {
                    if( is_numeric( $node->settings->meta_line_height['small']) && is_numeric( $node->settings->meta_font_size['small'] ) )
					$node->settings->meta_line_height_unit_responsive = round( $node->settings->meta_line_height['small'] / $node->settings->meta_font_size['small'], 2 );
				}
				if( isset( $node->settings->meta_line_height['medium']) && isset( $node->settings->meta_font_size['medium'] ) && $node->settings->meta_font_size['medium'] != 0 && !isset( $node->settings->meta_line_height_unit_medium ) ) {
                    if( is_numeric( $node->settings->meta_line_height['medium']) && is_numeric( $node->settings->meta_font_size['medium'] ) )
					$node->settings->meta_line_height_unit_medium = round( $node->settings->meta_line_height['medium'] / $node->settings->meta_font_size['medium'], 2 );
				}
				if( isset( $node->settings->meta_line_height['desktop']) && isset( $node->settings->meta_font_size['desktop'] ) && $node->settings->meta_font_size['desktop'] != 0 && !isset( $node->settings->meta_line_height_unit ) ) {
                    if( is_numeric( $node->settings->meta_line_height['desktop']) && is_numeric( $node->settings->meta_font_size['desktop'] ) )
					$node->settings->meta_line_height_unit = round( $node->settings->meta_line_height['desktop'] / $node->settings->meta_font_size['desktop'], 2 );
				}

				if ( isset( $node->settings->date_font_size['small']) && !isset( $node->settings->date_font_size_unit_responsive ) ) {
					$node->settings->date_font_size_unit_responsive = $node->settings->date_font_size['small'];
				}
				if( isset( $node->settings->date_font_size['medium']) && !isset( $node->settings->date_font_size_unit_medium ) ) {
					$node->settings->date_font_size_unit_medium = $node->settings->date_font_size['medium'];
				}
				if( isset( $node->settings->date_font_size['desktop']) && !isset( $node->settings->date_font_size_unit ) ) {
					$node->settings->date_font_size_unit = $node->settings->date_font_size['desktop'];
				}
				...
				if ( isset( $node->settings->taxonomy_filter_select_font_size['small']) && !isset( $node->settings->taxonomy_filter_select_font_size_unit_responsive ) ) {
					$node->settings->taxonomy_filter_select_font_size_unit_responsive = $node->settings->taxonomy_filter_select_font_size['small'];
				}
				if( isset( $node->settings->taxonomy_filter_select_font_size['medium']) && !isset( $node->settings->taxonomy_filter_select_font_size_unit_medium ) ) {
					$node->settings->taxonomy_filter_select_font_size_unit_medium = $node->settings->taxonomy_filter_select_font_size['medium'];
				}
				if( isset( $node->settings->taxonomy_filter_select_font_size['desktop']) && !isset( $node->settings->taxonomy_filter_select_font_size_unit ) ) {
					$node->settings->taxonomy_filter_select_font_size_unit = $node->settings->taxonomy_filter_select_font_size['desktop'];
				}
