...
class UABBFancyTextModule extends FLBuilderModule {
    ...
    function render_new_data( $data ) {
		...
		foreach ( $data as &$node ) {
			...
			if ( isset( $node->settings->type ) && 'fancy-text' === $node->settings->type ) {
				...
				if ( isset( $node->settings->fancy_font_size['small']) && !isset( $node->settings->fancy_font_size_unit_responsive ) ) {
					$node->settings->fancy_font_size_unit_responsive = $node->settings->fancy_font_size['small'];
				}
				if( isset( $node->settings->fancy_font_size['medium']) && !isset( $node->settings->fancy_font_size_unit_medium ) ) {
					$node->settings->fancy_font_size_unit_medium = $node->settings->fancy_font_size['medium'];
				}
				if( isset( $node->settings->fancy_font_size['desktop']) && !isset( $node->settings->fancy_font_size_unit ) ) {
					$node->settings->fancy_font_size_unit = $node->settings->fancy_font_size['desktop'];
				}

				if ( isset( $node->settings->fancy_line_height['small']) && isset( $node->settings->fancy_font_size['small'] ) && $node->settings->fancy_font_size['small'] != 0 && !isset( $node->settings->fancy_line_height_unit_responsive )) {
					if( is_numeric( $node->settings->fancy_line_height['small']) && is_numeric( $node->settings->fancy_font_size['small'] ) )
					$node->settings->fancy_line_height_unit_responsive = round( $node->settings->fancy_line_height['small'] / $node->settings->fancy_font_size['small'], 2 );
				}
				if( isset( $node->settings->fancy_line_height['medium']) && isset( $node->settings->fancy_font_size['small'] ) && $node->settings->fancy_font_size['medium'] != 0 && !isset( $node->settings->fancy_line_height_unit_medium )) {
					if( is_numeric( $node->settings->fancy_line_height['medium']) && is_numeric( $node->settings->fancy_font_size['small'] ) )
					$node->settings->fancy_line_height_unit_medium = round( $node->settings->fancy_line_height['medium'] / $node->settings->fancy_font_size['medium'], 2 );
				}
				if( isset( $node->settings->fancy_line_height['desktop']) && isset( $node->settings->fancy_font_size['small'] ) && $node->settings->fancy_font_size['desktop'] != 0 && !isset( $node->settings->fancy_line_height_unit )) {
					if( is_numeric( $node->settings->fancy_line_height['desktop']) && is_numeric( $node->settings->fancy_font_size['small'] ) )
					$node->settings->fancy_line_height_unit = round( $node->settings->fancy_line_height['desktop'] / $node->settings->fancy_font_size['desktop'], 2 );
				}
			}
		}
