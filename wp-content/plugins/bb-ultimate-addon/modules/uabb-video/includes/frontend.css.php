...
<?php if ( 'default' == $settings->play_source ) { 
	...
	<?php if('vimeo' === $settings->video_type) {
		if( isset( $settings->play_default_icon_bg ) ) { ?>
			.fl-node-<?php echo $id; ?> .uabb-play-viemo-button{
				<?php echo ('' != $settings->play_default_icon_bg ) ? 'background-color:' . $settings->play_default_icon_bg .';': 'background-color: rgba(0, 0, 0, 0.7);' ?>;?>
			}
		<?php } 
		if( isset( $settings->play_default_icon_bg_hover ) ) { ?>
			.fl-node-<?php echo $id; ?> .uabb-video__outer-wrap:hover .uabb-video__play-icon .uabb-play-viemo-button{
			<?php 
			 echo ('' != $settings->play_default_icon_bg_hover ) ? 'background-color:' . $settings->play_default_icon_bg_hover .';': 'background-color: rgba(0, 173, 239, 0.9);' ?>;
			}
		<?php } ?>
	<?php } ?>	
<?php } ?>