module.exports = function(grunt) {
    ...
    "/components/LogoList/bw-logo-list/"
    ],
    ...
    function get_downstream_custom_modules(){
        ...
        if ( grunt.file.exists( 'custom-modules-downstream.json' ) ){
            ...
            const downstream_custom_modules_file = grunt.file.readJSON( 'custom-modules-downstream.json' )
    ...
    function get_all_custom_modules( custom_modules_upstream ){
        ...
        const custom_modules_downstream = get_downstream_custom_modules()
        // By default, our module return is just the upstream modules
        let custom_modules_to_transpile = custom_modules_upstream
     ...
     */
     function get_module_name( file_path ){
        ...
        const file_path_array = file_path.replace(/\/$/, "").split('/')
        // Get the last item in the array (the name)
        const file_name = file_path_array[file_path_array.length - 1]
     ...
     */
     function get_less_object(){
        ...
        for ( let i=0; i<custom_modules.length; i++){
            ...
            const current_module_path = custom_modules[i]
     ...
     */
     function get_babel_object(){
        ...
        for ( let i=0; i<custom_modules.length; i++){
            ...
            const current_module_path = custom_modules[i]
            // The prebuilt path
            const prebuilt_js_file_path = '..'  + current_module_path + 'js/frontend.prebuilt.js'
            // The file path
            const frontend_js_file_path = '..'  + current_module_path + 'js/frontend.js'
    ...
    // Get the LESS object
    const less_items = get_less_object()
    // Get the uglify items
    const uglify_items   = get_uglify_object()
    uglify_items.options = { mangle: {keep_fnames : true} }
    // Get the CSS lint array
    const css_lint_array = get_css_lint_array()
    // Get the copy object
    const copy_object    = get_copy_object()
    // Get the JS Hint array
    const js_hint_array  = get_js_hint_array()
    // Get the JS Hint array
    const css_min_object = get_css_min_object()
    // Get the babel object
    const js_babel_object = get_babel_object()
    // Get the import object
    const js_import_object = get_import_object()
