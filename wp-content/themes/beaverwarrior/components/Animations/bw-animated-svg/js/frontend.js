 ...
 */
BWAnimatedSVG = function BWAnimatedSVG(settings) {
  // Set the settings
  ...
  this.animateOnScroll = settings.animateOnScroll; // Fire 'er up
...
BWAnimatedSVG.prototype = {
   ...
   * @type {Array}
   */
  unsupportedBrowsers: ['internet-explorer11'],
  ...
  init: function init() {
    ...
    this.lotteAnimationObject = this._getLottieAnimationObject(); // If we're on an older browser...
   ...
   */
  _getLottieAnimationObject: function _getLottieAnimationObject() {
    // Default params
    var params = {
   ...
   */
  _onUnsupportedBrowser: function _onUnsupportedBrowser() {
   ...
   * @return {void}
   */
  _removeHeightStyle: function _removeHeightStyle() {
    ...
    new_style_array = []; // Loop through the styles

    for (var i = 0; i < existing_style_array.length; i++) {
      // Get the current style
      var current_style = existing_style_array[i].trim(),
          // Get the key for this style
      style_key = current_style.split(':')[0]; // If the style key is anything besides height, then add it back
    ...
    var new_style_attribute = new_style_array.join('; ') + ';'; // Set the new style attribute
  ...
  _playAnimation: function _playAnimation() {
    // Here we go!
    this.lotteAnimationObject.play(); // Mark the animation as complete
   ...
   * @returns {void}
   */
  _bindIntersectionObserver: function _bindIntersectionObserver() {
